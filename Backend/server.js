...
// Connect to MongoDB
mongoose.connect('mongodb+srv://priyanshuchp123:Aw7EvcfAPRXwbKyu@cluster0.nm8ct.mongodb.net/auctionDB')
...
app.put('/auctions/:id', authenticate, async (req, res) => {
  try {
    const updatedItem = await AuctionItem.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!updatedItem) return res.status(404).json({ message: 'Auction item not found' });
    res.json({ message: 'Auction item updated', item: updatedItem });
  } catch (error) {
    console.error('Updating Auction Item Error:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
});
...
app.delete('/auctions/:id', authenticate, async (req, res) => {
  try {
    ...
    if (!deletedItem) return res.status(404).json({ message: 'Auction item not found' });
